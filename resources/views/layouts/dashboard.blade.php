...
<body>
        ...
        <nav class="navbar navbar-expand-md shadow-sm">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                ...
                <ul class="navbar-nav ml-auto align-items-center">
                    <li class="nav-item">
                        <span class="nav-link">
                            <img id="selfCountryflag" src="" alt="{{ __('Country Flag') }}" width="25"
                                class="mr-2" hidden />
                            <i id="online" class="fas fa-circle"></i> <span id="onlineCount">X</span>
                            {{ __('Live') }}
    ...
    <script>
        ...
        const languages = {
            ...
            no_session_details: "{{ __('Could not get the session details') }}",
            system: "{{ __('System') }}",
            cant_connect_server: "{{ __('Could not connect to the server, please try refreshing the page') }}",
            banned: "{{ __('Banned') }}",
            no_device: "{{ __('Could not get the devices, please check the error and try again') }}",
            user_reported: "{{ __('The user has been reported') }}",
            you_banned: "{{ __('You are banned') }}",
            oops: "{{ __('Oops') }}",
            upgrade_now: "{{ __('Upgrade now to use this feature') }}",
            upgrade: "{{ __('Upgrade') }}",
            feature: "{{ __('Feature') }}",
            user_left: "{{ __('The user has left the chat, searching for a new partner') }}",
            searching: "{{ __('Searching for a partner') }}",
            connected_with: "{{ __('You are now connected with') }}"
        };
    </script>
